                    ...
                    <table class="table table-vcenter card-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $service->id }}</td>
                                <td>{{ $service->title }}</td>
                                <td>{{ Str::limit($service->subtitle, 50) }}</td>
                                <td>
                                    <img src="{{ asset($service->iamge) }}" width="100">
                                </td>
                                <td>
                                    <!-- Edit -->
                                    <a href="{{ route('services.edit', $service->id) }}" class="btn-sm btn-primary">
                                    ...
                                    <form action="{{ route('services.destroy', $service->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-sm btn-danger" onclick="return confirm('Are you sure to delete this service?')">
